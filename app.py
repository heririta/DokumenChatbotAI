...
def render_sidebar():
    ...
    if current_page not in ['chat', 'documents']:
        ...
        if groq_api_key:
            ...
            st.sidebar.info("Please set it in your .env file")

        # LLM Provider - Google only
        st.sidebar.info("🤖 **LLM Provider:** Groq ")
...
def render_homepage():
    ...
    with logo_col2:
        try:
            st.image("assets/Google__logo.png", width=120, use_container_width=False)
        except:
            pass  # If image not found, continue without it
    ...
    # tech_logos = ["assets/Google__logo.png", "assets/hacktiv8.htm"]
...
def main():
    ...
    st.markdown(
        ...
        "🤖 Document Chatbot Heri Santoso | Powered by Google "
        "</div>",
        unsafe_allow_html=True
    )
